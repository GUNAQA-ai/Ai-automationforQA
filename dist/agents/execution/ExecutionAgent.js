...
const console_ui_1 = require("../../utils/console-ui");
/**
...
class ExecutionAgent {
    ...
    async run(specPath) {
        const absolutePath = path_1.default.resolve(specPath);
        const relativePath = path_1.default.relative(process.cwd(), absolutePath).split(path_1.default.sep).join('/');
        this.logger.info(`ExecutionAgent: running spec ${absolutePath}`);
        (0, console_ui_1.executionLog)('info', 'Execution started', `Spec: ${relativePath}`);
        return new Promise((resolve, reject) => {
            ...
            (0, console_ui_1.executionLog)('action', 'Opening browser', `Project: ${project}${modeFlag ? `, mode:${modeFlag.trim()}` : ''}`);
            let stdout = '';
            let stderr = '';
            let timedOut = false;
            const configuredTimeout = Number(process.env.EXECUTION_TIMEOUT_MS ?? 80000);
            const executionTimeout = Number.isFinite(configuredTimeout) && configuredTimeout > 0 ? configuredTimeout : 80000;
            const child = (0, child_process_1.spawn)(cmd, { cwd: process.cwd(), shell: true, env: process.env });
            ...
            child.on('error', (error) => {
                ...
                const frameworkError = new FrameworkError_1.FrameworkError('Execution process failed to start', error, 'EXEC_START_FAIL');
                (0, console_ui_1.executionLog)('error', 'Execution failed', 'Playwright process could not start');
            ...
            child.on('close', (code) => {
                ...
                if (code !== 0 || timedOut) {
                    ...
                    if (failedSelector) {
                        ...
                        (0, console_ui_1.executionLog)('heal', 'Healing candidate detected', failedSelector);
                    }
                    else {
                        (0, console_ui_1.executionLog)('skip', 'Healing skipped', failure.reason);
                    }
                    const originalError = new Error(timedOut ? 'Playwright execution timed out' : `Playwright exited with code ${code}`);
                    const frameworkError = new FrameworkError_1.FrameworkError('Execution failed', originalError, 'EXEC_FAIL');
                    ...
                    (0, console_ui_1.executionLog)('success', 'Execution passed', 'All browser steps completed');
    ...
    extractFailedSelector(output) {
        ...
        if (quotedLocatorLineMatch?.[2])
            return this.cleanExtractedSelector(quotedLocatorLineMatch[2]);
        const quotedWaitingForLocatorMatch = output.match(/waiting for locator\(\s*(['"`])([\s\S]*?)\1\s*\)/i);
        if (quotedWaitingForLocatorMatch?.[2])
            return this.cleanExtractedSelector(quotedWaitingForLocatorMatch[2]);
        const quotedLocatorMatch = output.match(/locator\(\s*(['"`])([\s\S]*?)\1\s*\)/i);
        if (quotedLocatorMatch?.[2])
            return this.cleanExtractedSelector(quotedLocatorMatch[2]);
        ...
        if (locatorLineMatch?.[1])
            return this.cleanExtractedSelector(locatorLineMatch[1]);
        const waitingForLocatorMatch = output.match(/waiting for locator\((.+?)\)/i);
        if (waitingForLocatorMatch?.[1])
            return this.cleanExtractedSelector(waitingForLocatorMatch[1]);
        const locatorMatch = output.match(/locator\((.+?)\)/);
        if (locatorMatch?.[1])
            return this.cleanExtractedSelector(locatorMatch[1]);
        const rawLocatorLineMatch = output.match(/Locator:\s+(.+)/i);
        if (rawLocatorLineMatch?.[1])
            return this.cleanExtractedSelector(rawLocatorLineMatch[1]);
        const frameworkActionMatch = output.match(/(?:FrameworkError:\s*)?(?:clickElement|clickIfVisible|enterText|clearText|selectDropdownByValue|selectDropdownByText|enterTextAndSelectOption|selectCheckbox|unselectCheckbox|hoverOverElement|doubleClickElement|rightClickElement|waitForElementVisible|waitForElementClickable|waitForTextPresent|verifyVisible|verifyEnabled|verifyText|acceptAlert|dismissAlert|click|fill|press|select|selectByText|fillAndChoose|check|uncheck|hover|dragAndDrop|uploadFile)\s+failed on\s+([^\r\n]+)/i);
        if (frameworkActionMatch?.[1])
            return this.cleanExtractedSelector(frameworkActionMatch[1]);
        const waitingForMatch = output.match(/waiting for (?:locator\()?['"`]([^'"`\n]+)['"`]\)?/i);
        if (waitingForMatch?.[1])
            return this.cleanExtractedSelector(waitingForMatch[1]);
