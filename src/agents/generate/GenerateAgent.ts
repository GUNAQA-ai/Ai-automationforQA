...
interface GeneratedArtifactIndex {
  version: number;
  entries: GeneratedArtifactEntry[];
}
...
export class GenerateAgent {
  ...
  private readonly artifactIndexPath = path.resolve('generated', '.artifact-index.json');
  private readonly apiStatePath = path.resolve('storage', 'api-state.json');

  async run(planPath: string, executionError?: string): Promise<string> {
    try {
      let plan = await this.applyApiStateToPlan(JSON.parse(await readFile(planPath, 'utf-8')));
      ...
      let validationError = '';
      let lastPromptSpec = '';
      let lastSupportFiles: Record<string, string> = {};
      ...
      while (attempt < MAX_RETRIES) {
        ...
        let generatedCode = this.ensureMinimumTestTimeout(this.addExecutionLogsToSpec(
        ...
        const filePlan = await this.planGeneratedWrites(supportFiles, `${sanitizedScenarioName}.spec.ts`, plan);
        const remappedSupportFiles = this.remapSupportFiles(supportFiles, filePlan.supportFileNames);
        const remappedGeneratedCode = this.rewriteGeneratedImportsForUniqueFiles(generatedCode, filePlan.supportFileNames);
        const specPath = path.join(this.testsDir, filePlan.specFileName);
        ...
        await writeFile(specPath, remappedGeneratedCode);

        // Check compilation programmatically
        this.logger.info(`GenerateAgent: Running TypeScript compilation check on attempt ${attempt}...`);
        const compResult = await new Promise<{ success: boolean; errors: string }>((resolve) => {
          exec('npx tsc -p tsconfig.generated.json --noEmit', { cwd: process.cwd() }, (error, stdout, stderr) => {
            if (error) {
        ...
        if (compResult.success) {
          ...
          // Clean up failed files from disk
          this.logger.info(`GenerateAgent: Cleaning up failed compilation files from disk before retry...`);
          try {
            await remove(specPath);
            for (const remappedName of Object.keys(remappedSupportFiles)) {
              const isLocatorFile = this.isLocatorSupportFile(remappedName, remappedSupportFiles[remappedName]);
              const targetDir = isLocatorFile ? this.locatorsDir : this.pagesDir;
              await remove(path.join(targetDir, remappedName));
              if (isLocatorFile && remappedName.endsWith('.ts')) {
                await remove(path.join(targetDir, remappedName.replace(/\.ts$/, '.json')));
              }
            }
          } catch (cleanupErr) {
      ...
      // If loop completes, fallback to local structured fallback
  ...
  private scenarioText(value: unknown, fallback = 'Generated scenario'): string {
  ...
  private async applyApiStateToPlan(plan: any): Promise<any> {
    ...
    if (JSON.stringify(resolvedPlan) !== JSON.stringify(plan)) {
      this.logger.info(`GenerateAgent: resolved plan placeholders from ${path.relative(process.cwd(), this.apiStatePath)}`);
    }

    return resolvedPlan;
  }

  private async readApiState(): Promise<Record<string, any> | undefined> {
    try {
      if (!await pathExists(this.apiStatePath)) return undefined;
      const parsed = JSON.parse(await readFile(this.apiStatePath, 'utf-8'));
  ...
  private resolveTemplatesInObject(value: unknown, context: Record<string, unknown>): any {
  ...
  private resolveTemplateString(value: string, context: Record<string, unknown>): unknown {
  ...
  private lookupTemplateValue(expression: string, context: Record<string, unknown>): unknown {
  ...
  private readContextPath(value: unknown, selector: string): unknown {
  ...
  private async findReusableGeneratedSpec(plan: any, planFingerprint: string): Promise<string | undefined> {
    const index = await this.readArtifactIndex();
    const scenarioKey = this.scenarioKey(plan);
    const reusable = index.entries.find((entry) => (
      entry.planFingerprint === planFingerprint
    ...
    if (!allFiles.every((file) => this.isShortVisibleFileName(file))) return undefined;

    const allExist = (await Promise.all(allFiles.map((file) => pathExists(file)))).every(Boolean);
  ...
  private async recordGeneratedArtifact(
    ...
    const supportFilePaths = Object.keys(supportFiles).map((fileName) => {
      ...
      return this.relativePath(path.join(targetDir, fileName));
    });

    const nextEntry: GeneratedArtifactEntry = {
      planFingerprint,
      scenarioKey,
    ...
    await writeFile(this.artifactIndexPath, JSON.stringify(index, null, 2));
  }

  private async readArtifactIndex(): Promise<GeneratedArtifactIndex> {
    try {
      const raw = await readFile(this.artifactIndexPath, 'utf-8');
      const parsed = JSON.parse(raw);
      if (parsed && typeof parsed === 'object' && Array.isArray(parsed.entries)) {
        return {
          version: Number(parsed.version) || 1,
          entries: parsed.entries.filter((entry: unknown): entry is GeneratedArtifactEntry => Boolean(entry && typeof entry === 'object')),
        };
      }
    } catch {
  ...
  private resolveIndexedPath(filePath: string): string {
  ...
  private planFingerprint(plan: any): string {
    const stablePlan = {
  ...
  private scenarioKey(plan: any): string {
...
`;
  ...
  private buildStructuredSpec(plan: any, pageClass: string, locatorExport: string, locators: Record<string, string>): string {
    ...
    const methodNames = this.buildMethodNamesForSteps(steps);
    const body = steps
      .map((step: any, index: number) => this.buildStructuredStep(step, index, pageVar, locatorExport, locators, plan.testData, methodNames[index], testDataExport))
      .filter(Boolean)
      .join('\n\n');

    let extraImports = '';
    const hasApi = steps.some((s: any) => String(s.action).toLowerCase() === 'api');
    const hasData = steps.some((s: any) => ['db', 'querydb', 'generaterandomdata', 'validatepdf', 'validateexcel', 'validatezip'].includes(String(s.action).toLowerCase()));
...
`;
  ...
  private navigationTargetName(target: string): string {
    try {
  ...
  private filterStepsToResolvedLocators(steps: any[], locators: Record<string, string>, testData?: Record<string, any>): any[] {
  ...
  private stepRequiresLocator(step: any, testData?: Record<string, any>): boolean {
    const action = String(step?.action ?? '').toLowerCase();
  ...
  private async discoverLocatorsFromDom(plan: any, baseLocators: Record<string, string>): Promise<Record<string, string>> {
    return {};
  }
  ...
  private stepValueExpression(step: any, value: any, testData?: Record<string, any>, testDataExport?: string): string {
  ...
  private findTestDataKeyForStep(step: any, value: any, testData?: Record<string, any>): string | undefined {
    if (!testData || value === undefined || value === null || Array.isArray(value)) return undefined;
  ...
  private objectAccess(key: string): string {
  ...
  private friendlyStepTitle(step: any, index: number, methodName: string): string {
  ...
  private buildLocatorsFromPlan(
    ...
    const className = exportName.endsWith('Locators')
      ? exportName.slice(0, -'Locators'.length)
      : exportName;
...
`;
  ...
  private testDataExportName(locatorExport: string): string {
    return locatorExport.endsWith('Locators')
      ? `${locatorExport.slice(0, -'Locators'.length)}TestData`
      : `${locatorExport}TestData`;
  }

  private stringifyObjectLiteral(value: Record<string, any>): string {
  ...
  private selectorIntentMismatch(oldSelector: string, newSelector: string): boolean {
  ...
  private selectorIntent(selector: string): string | undefined {
  ...
  private deriveClassName(scenario: unknown): string {
  ...
  private validateGeneratedCode(code: string, supportFiles: Record<string, string> = {}, plan?: { steps?: any[]; testData?: Record<string, any> }): void {
    if (!code) {
      throw new FrameworkError('Generated code is empty', undefined, 'GEN_EMPTY');
    }

    if (!this.isValidGeneratedCode(code, plan, supportFiles)) {
      throw new FrameworkError(`Generated code does not contain a runnable Playwright test: ${this.getInvalidReason(code, plan, supportFiles)}`, undefined, 'GEN_INVALID');
    }
  }

  private isValidGeneratedCode(code: string, plan?: { steps?: any[]; testData?: Record<string, any> }, supportFiles: Record<string, string> = {}): boolean {
    return Boolean(
      code &&
  ...
  private getInvalidReason(code: string, plan?: { steps?: any[]; testData?: Record<string, any> }, supportFiles: Record<string, string> = {}): string {
  ...
  private generatedCodeCoversPlan(code: string, plan?: { steps?: any[]; testData?: Record<string, any> }, supportFiles: Record<string, string> = {}): boolean {
    return !this.getPlanCoverageFailure(code, plan, supportFiles);
  }

  private getPlanCoverageFailure(code: string, plan?: { steps?: any[]; testData?: Record<string, any> }, supportFiles: Record<string, string> = {}): string | undefined {
  ...
  private executablePlanSteps(plan?: { steps?: any[]; testData?: Record<string, any> }): any[] {
    return (Array.isArray(plan?.steps) ? plan.steps : [])
      .filter((step) => !this.isDataOnlyItemsStep(step, plan?.testData));
  }

  private coverageText(contents: string[]): string {
    return contents
      .join('\n')
  ...
  private includesCoveragePhrase(text: string, value: string): boolean {
    const phrase = this.coverageText([value]);
    return Boolean(phrase) && text.includes(phrase);
  }

  private requiredActionFamilies(steps: any[]): string[] {
    return Array.from(new Set(steps.map((step) => this.actionFamily(step)).filter((family): family is string => Boolean(family))));
  }

  private actionFamily(step: any): string | undefined {
    const action = String(step?.action ?? '').toLowerCase();
  ...
  private coverageHasActionFamily(text: string, family: string): boolean {
  ...
  private usesMissingFrameworkActions(supportFiles: Record<string, string>): boolean {
    return Object.entries(supportFiles)
      .filter(([fileName]) => /page/i.test(fileName))
  ...
  private async planGeneratedWrites(
    supportFiles: Record<string, string>,
    specFileName: string,
    plan: any
  ): Promise<{ supportFileNames: Record<string, string>; specFileName: string }> {
    const reserved = new Set<string>();
    const supportFileNames: Record<string, string> = {};
  ...
  private async generatedFileNameForScenario(targetDir: string, fileName: string, reserved: Set<string>, plan: any): Promise<string> {
    const ext = fileName.endsWith('.spec.ts') ? '.spec.ts' : path.extname(fileName);
    const base = this.shortGeneratedBaseName(path.basename(fileName, ext), ext, '');
    const candidate = `${base}${ext}`;
    const candidatePath = path.join(targetDir, candidate);

    if (!reserved.has(candidatePath) && await pathExists(candidatePath) && !await this.indexedToDifferentScenario(candidatePath, plan)) {
  ...
  private async indexedToDifferentScenario(filePath: string, plan: any): Promise<boolean> {
    const index = await this.readArtifactIndex();
  ...
  private async uniqueGeneratedFileName(targetDir: string, fileName: string, reserved: Set<string>): Promise<string> {
    const ext = fileName.endsWith('.spec.ts') ? '.spec.ts' : path.extname(fileName);
    const base = this.shortGeneratedBaseName(path.basename(fileName, ext), ext, '');
    ...
    // Only append suffix if it collides with a file generated IN THIS EXACT RUN (reserved)
    while (reserved.has(candidatePath)) {
      const suffix = `_${index}`;
      candidate = `${this.shortGeneratedBaseName(path.basename(fileName, ext), ext, suffix)}${suffix}${ext}`;
  ...
  private shortGeneratedBaseName(baseName: string, ext: string, suffix: string): string {
  ...
  private isShortVisibleFileName(file: string): boolean {
    return path.basename(file).length <= 16;
  }

  private remapSupportFiles(files: Record<string, string>, fileNameMap: Record<string, string>): Record<string, string> {
    return Object.fromEntries(
  ...
  private rewriteGeneratedImportsForUniqueFiles(code: string, fileNameMap: Record<string, string>): string {
    return code.replace(/from\s+(['"])(\.{1,2}\/[^'"]+)\1/g, (match, quote, importName) => {
      ...
      const importDir = path.posix.dirname(normalizedImport);
      const newBase = path.basename(newFileName, path.extname(newFileName));
      const isJson = oldFileName.endsWith('.json');
      const newImport = importDir === '.' 
        ? `./${newBase}${isJson ? '.json' : ''}` 
        : `${importDir}/${newBase}${isJson ? '.json' : ''}`;
  ...
  private isLocatorSupportFile(fileName: string, content = ''): boolean {
